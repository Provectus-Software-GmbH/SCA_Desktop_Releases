...
1. Runs [Sync-SecureContactsToIntune.sh](Scripts/Sync-SecureContactsToIntune.sh), which downloads the latest stable ARM64 PKG and matching SHA256 file from the official GitHub release and validates it without Graph access.
...
The package validator must run on macOS. It uses Apple tools including `pkgutil`, `spctl`, `plutil`, and `PlistBuddy`. A Windows agent cannot replace the macOS agent for this workflow.
...
The sync script downloads releases from this public repository without a GitHub token. If your organization changes the release visibility, set `GITHUB_API_URL` and `GITHUB_REPOSITORY` or provide the required authenticated network access through the runner. Do not print tokens in logs.
...
The pipeline uses the sync script to download and validate the stable release on macOS, then publishes non-secret validation artifacts. It does not call Microsoft Graph.
...
The validation stage must succeed first. Azure DevOps then starts a deployment job associated with `intune-production`; the configured environment approval must pass before the existing Intune app is updated. The publish job invokes the self-contained `Sync-SecureContactsToIntune.sh --publish` flow.
