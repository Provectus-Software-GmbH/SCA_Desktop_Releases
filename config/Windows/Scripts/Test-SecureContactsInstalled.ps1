...
[CmdletBinding()]
param(
    [string]$AppName = "SecureContacts",
    [string]$MinimumVersion = "0.8.0"
)

$ErrorActionPreference = 'SilentlyContinue'
$AcceptedAppNames = @('SecureContacts', 'Secure Contacts')
if (-not [string]::IsNullOrWhiteSpace($AppName) -and $AppName -notin $AcceptedAppNames) {
    $AcceptedAppNames += $AppName
}
...
# 2. Query registry for the exact application DisplayName
$InstalledApp = Get-ItemProperty -Path $UninstallPaths | 
    Where-Object { $_.DisplayName -in $AcceptedAppNames } | 
...
if ($InstalledApp) {
    ...
    try {
        ...
        # Invalid version metadata must not be treated as compliant.
        exit 1
    }
} else {
