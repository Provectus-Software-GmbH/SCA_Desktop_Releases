...
2. Downloads the latest stable ARM64 PKG and matching SHA256 file from the official GitHub release.
3. Runs [Validate-SecureContactsPackage.sh](Scripts/Validate-SecureContactsPackage.sh) and the shared sync script with `--skip-recipe`.
...
The release download uses the workflow's GitHub token with read-only repository contents permission. The workflow expects the official public release assets. Do not replace the stable-tag and exact filename checks with arbitrary tags or URLs.
...
The validation job must succeed. The publish job then waits for the protected `intune-production` environment approval, downloads the exact validated artifact, and invokes `Sync-SecureContactsToIntune.sh --skip-recipe --publish`.
