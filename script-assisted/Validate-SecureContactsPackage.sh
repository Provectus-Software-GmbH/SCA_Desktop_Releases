...
usage() {
  cat <<'EOF'
Usage: Validate-SecureContactsPackage.sh [--output PATH] [--manifest-output PATH]

Validates an existing signed Secure Contacts ARM64 PKG and its SHA-256 checksum.
This script does not build or download packages and never writes to Microsoft Graph.
Run it on macOS with the Apple package tools.
EOF
}
